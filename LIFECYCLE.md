...
|---|---|---|---|
| §8 종료 독립 검수 · 6a plan 검수(시험 적용) | `design-doc-reviewer` | high | **대조** — 판정 기준(output-template / 스펙 문서·Verified Facts)이 문서 밖에 이미 있다. 정답지를 들고 하는 작업 |
...
```
**plan 검수 (시험 적용, n=0)**: plan 이 나오면 메인이 suite `design-doc-reviewer` 를 1회 디스패치한다 — 전달은 plan + 상류 스펙 문서 경로만(작성 서사 금지 — §8 과 동일 규율). 판정 축 4개: ① **Verified Facts 실코드 대조**(PRINCIPLES §11 — 표의 시그니처·반환 계약이 실제 코드와 일치하나) ② **스펙↔태스크 커버리지**(스펙 항목이 태스크에 다 배정됐나, 누락·임의 축소 없나) ③ **파일 목록·PR 경계 선언 여부**(위 프롬프트가 요구한 것을 실제로 냈나) ④ **Open Questions 위장**(미결이 임의 결정으로 채워져 숨지 않았나). 지적은 항목별 수용/기각, 재검수 없음(bounded). 근거: 계획서 오류 7건이 구현 단계의 태스크 구현자 mutation 실측에서야 발견된 사례 — TDD oracle 이 잡긴 하지만 그 시점엔 태스크가 이미 그 오류 위에 서 있어, 가장 비싼 발견 지점이다. dogfood 1줄 기록, 거취는 §8 과 동일(2회 연속 ⓐ 없음이면 제거 — DESIGN 거취 트리거 보드).
