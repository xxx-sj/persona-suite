...
각 repo의 `.claude/persona-profile.md` 에 용량 제약·출력 위치·핸드오프 타깃·관측/로깅 관례·횡단 정책을 둔다. **없으면 첫 실행 때 스킬이 자동 생성**한다 (CLAUDE.md 등에서 추론 + 빈칸만 질문 → 파일로 저장). 프로필이 없어도 스킬은 물으면서 동작한다.

같은 위치의 `.claude/persona-feedback.md` 는 **역주입 큐**다 — 감사(8.5)의 목록외 발견이나 운영 사고를 한 줄로 적어두고, 플러그인 repo 를 열 때 비우면서 `checklist.md`·질문뱅크에 반영한다. 발견 시점(프로젝트)과 편입 시점(플러그인)이 갈리는 걸 잇는 장치 ([`LIFECYCLE.md`](./LIFECYCLE.md) §13).