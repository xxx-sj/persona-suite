...
**테스트 리스트 미지정이면 핑퐁(학습) 모드가 기본** — 스킬이 inventory 를 비공개로 선고정한 뒤 "어떤 테스트를 짤래?"를 되묻고, 사용자 리스트 판정 후 공개 대조한다 (스킬 §핑퐁 모드 — plan 의 테스트 섹션을 열기 *전*에). 연습 없이 검증만 원하면 리스트를 지정해 부르거나, 되묻기에 "그냥 판정만"으로 강등한다.
