...
## 공유 원칙 로드 (suite 공통)
플러그인 root [`PRINCIPLES.md`](../../PRINCIPLES.md) 를 읽어 적용한다 — 피드백 모드(결정만 사용자)·개념 본질 도출·right-size·챕터 자기점검(시작 재선언+트리거 점검)·specifics 가정 금지·용어·결정 번복 절차(문서 먼저)·결정 스팟 반박(무거움 tier §10). 이 스킬 변형: **결정 = 구조·패턴 선택**, right-size = **추상화는 2회 이상 반복 시만(YAGNI)**.
...
5. 에러 처리 + 기록 구조 — 어디서 throw/catch? 관찰 가능한 결과로 끝나나? **어디서 남기나**(프로필 `observability.로깅 규약` 준수 — 계층·레벨·상관관계 키·마스킹). catch 후 기록만 하고 삼키면 실패가 성공처럼 보이고, 계층마다 같은 에러를 남기면 원인 1건이 알람 N건이 된다
6. [REV] 종료 자기 점검 — 빠진 거·과설계 훑기
6.5 (무거움 tier) [ARCH] 스케일 라운드 **1회 offer** — 공유 원칙 §3 학습 모드(no-pre-fill). 넘기면 §7 스케일 경로 칸만 기록. 라운드 결과가 "지금 그 구조로 가자"면 스코프 크립 — 시야만 넓히고 이번 구현은 최단순 유지
7. 산출 → 코드 설계 doc (output-template)
8. 종료 독립 검수 (공유 원칙 §8) — 완성 문서 독립 서브에이전트 1회 → handoffs.plan 핸드오프
```
