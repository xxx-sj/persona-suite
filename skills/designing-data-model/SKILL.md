...
## 공유 원칙 (suite 공통 — defining-requirements와 동일)
- **피드백 모드(개발 단계)**: 사용자가 제안 → 페르소나가 빠진 곳·약한 곳 지적 + 검증 + *도출 초안* 제안. **결정(모델·인덱스 선택)만은 사용자가 커밋.** 순수 Socratic 아님.
- **개념은 본질에서 도출**(모르면 "이게 뭐야?"). **관심사는 적되 솔루션 처방 X** — 해법은 프로필 `capacity` 규모서 *최단순*부터 (인덱스 남발·조기 샤딩 금지).
- **챕터 단위 자기 점검 루프**. 확정된 결정은 "**확정**"으로 표기("락"은 기술적 락과 혼동되니 금지).
...
## 진행 절차
```
0. 입력 로드 + right-size (작은 스키마 변경이면 [REV][DBA] 빠른 패스)
...
8. 산출 → 데이터 모델 설계 doc (output-template) → handoffs.plan 핸드오프
```
