...
5종. `[REV]`(모델링) 메인, 트리거로 나머지 호출. 개별 호명 가능.

**공통 규칙**: root [`PRINCIPLES.md`](../../PRINCIPLES.md) (SKILL.md 단계에서 로드) — 피드백 모드·개념 본질 도출·right-size(capacity 최단순)·페르소나 공통 규칙(컨텍스트 공유·영역 침범 X·막히면 좁은 질문/"모름").
...
- (스케일 학습 모드 — PRINCIPLES §3, 무거움 tier 산출 직전 1회 offer) "문서 수가 10배·100배면 **뭐가 먼저 무너져?** 쓰기 지연? 인덱스가 메모리를 벗어남? 스캔?" → 사용자 답 후 "그때 후보는 뭐가 있고(인덱스 재설계 / 읽기 분리 / 아카이빙·파티셔닝 / 샤딩) **각각 데이터를 옮겨야 하나, 쿼리 어디가 바뀌나?**" — 후보를 먼저 펼치지 말고 사용자 답에 빠진 축만 보탠다. 기각 후보도 같은 축을 채운 뒤 기각
